...
class SchemaParser:
    ...
    def get_relations(self, object_type: str | None = None) -> dict[str, list[str]] | list[str]:
        ...
        for i, capture in enumerate(captures):
            ...
            if name == "def_name":
                ...
                if current_def not in relations:
                    relations[current_def] = []
            
            elif name == "rel_name" and (object_type is None or current_def == object_type):
    ...
    def get_permissions(self, object_type: str | None = None) -> dict[str, list[str]] | list[str]:
        ...
        for capture in captures:
            ...
            if name == "def_name":
                current_def = node.text.decode("utf-8")
                if current_def not in permissions:
                    permissions[current_def] = []
            
            elif name == "perm_name" and (object_type is None or current_def == object_type):
