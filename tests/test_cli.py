...
def setup_cli_test(mocker, args=None, path_exists=True, has_diff=False, schema_path=None, output_path=None):
    ...
    mocker.patch("authz_schema_sync_check.cli.TypeGenerator.generate_types", 
                return_value="generated content")
    
    # Mock get_diff
    diff_output = "diff output" if has_diff else ""
    mock_get_diff = mocker.patch("authz_schema_sync_check.cli.get_diff", 
                                return_value=(has_diff, diff_output))
...
def test_cli_with_auto_fix(mocker):
    ...
    args = ["check-schema", f"--schema={FIXTURES_DIR / 'valid_schema.zed'}", f"--output=models.py", "--auto-fix"]
...
def test_cli_verbose(mocker):
    ...
    args = ["check-schema", f"--schema={FIXTURES_DIR / 'valid_schema.zed'}", f"--output=models.py", "--verbose"]
...
def test_cli_with_colorized_diff_enabled(mocker):
    ...
    args = ["check-schema", f"--schema={FIXTURES_DIR / 'valid_schema.zed'}", f"--output=models.py", "--colorized-diff=True"]
    mocks = setup_cli_test(mocker, args=args, has_diff=True)
    
    # Mock colorize_diff to track if it's called
    mock_colorize_diff = mocker.patch("authz_schema_sync_check.cli.colorize_diff", return_value="colorized diff")
...
def test_cli_with_colorized_diff_disabled(mocker):
    ...
    args = ["check-schema", f"--schema={FIXTURES_DIR / 'valid_schema.zed'}", f"--output=models.py", "--colorized-diff=False"]
    mocks = setup_cli_test(mocker, args=args, has_diff=True)
